...
class SearchPosterListingAdapter(
    private val list: ArrayList<PosterItemDTO.Page.ContentItems.Content> = arrayListOf()) :

    RecyclerView.Adapter<SearchPosterListingAdapter.PosterViewHolder>() {
    ...
    fun addData(data: List<PosterItemDTO.Page.ContentItems.Content>, searchQuery : String) {
    ...
    inner class PosterViewHolder(
        ...
        fun bind(item: PosterItemDTO.Page.ContentItems.Content) {
            ...
            // Find the start and end indices of the search query in the item
            val startIndex = item.name.indexOf(searchQuery, ignoreCase = true)
            val endIndex = startIndex + searchQuery.length

            if (startIndex != -1) {
                val highlightColor = ContextCompat.getColor(binding.root.context, R.color.yellow)
            ...
            binding.tvPoster.text=spannableString
        }
    }
